...
int nAnonymizeMasterCoinAmount = 1000;
...
#else
    const char* pszModule = "MasterCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterCoin
    // Mac: ~/Library/Application Support/MasterCoin
    // Unix: ~/.MasterCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MasterCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MasterCoind.pid"));
