...
/* end documentation of inline functions */
/* start documentation of MasterCoin virtual functions */
...
/* end documentation of MasterCoin virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the MasterCoin
...
/* start of documentation of MasterCoin virtual functions */
...
/* end of documentation of MasterCoin virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the MasterCoin
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following MasterCoin virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs MasterCoinping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *MasterCoinpedData = &otherData;
  
  // MasterCoin both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (MasterCoinpedData->first().x() > MasterCoinpedData->last().x())
    {
      int size = MasterCoinpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*MasterCoinpedData)[i], (*MasterCoinpedData)[size-1-i]);
    }
    // MasterCoin lower bound:
    if (staticData->first().x() < MasterCoinpedData->first().x()) // other one must be MasterCoinped
      qSwap(staticData, MasterCoinpedData);
    int lowBound = findIndexBelowX(MasterCoinpedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    MasterCoinpedData->remove(0, lowBound);
    // set lowest point of MasterCoinped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (MasterCoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (MasterCoinpedData->at(1).x()-MasterCoinpedData->at(0).x() != 0)
      slope = (MasterCoinpedData->at(1).y()-MasterCoinpedData->at(0).y())/(MasterCoinpedData->at(1).x()-MasterCoinpedData->at(0).x());
    else
      slope = 0;
    (*MasterCoinpedData)[0].setY(MasterCoinpedData->at(0).y()+slope*(staticData->first().x()-MasterCoinpedData->at(0).x()));
    (*MasterCoinpedData)[0].setX(staticData->first().x());
    
    // MasterCoin upper bound:
    if (staticData->last().x() > MasterCoinpedData->last().x()) // other one must be MasterCoinped
      qSwap(staticData, MasterCoinpedData);
    int highBound = findIndexAboveX(MasterCoinpedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    MasterCoinpedData->remove(highBound+1, MasterCoinpedData->size()-(highBound+1));
    // set highest point of MasterCoinped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (MasterCoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = MasterCoinpedData->size()-1; // last index
    if (MasterCoinpedData->at(li).x()-MasterCoinpedData->at(li-1).x() != 0)
      slope = (MasterCoinpedData->at(li).y()-MasterCoinpedData->at(li-1).y())/(MasterCoinpedData->at(li).x()-MasterCoinpedData->at(li-1).x());
    else
      slope = 0;
    (*MasterCoinpedData)[li].setY(MasterCoinpedData->at(li-1).y()+slope*(staticData->last().x()-MasterCoinpedData->at(li-1).x()));
    (*MasterCoinpedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (MasterCoinpedData->first().y() < MasterCoinpedData->last().y())
    {
      int size = MasterCoinpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*MasterCoinpedData)[i], (*MasterCoinpedData)[size-1-i]);
    ...
    // MasterCoin lower bound:
    if (staticData->first().y() > MasterCoinpedData->first().y()) // other one must be MasterCoinped
      qSwap(staticData, MasterCoinpedData);
    int lowBound = findIndexAboveY(MasterCoinpedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    MasterCoinpedData->remove(0, lowBound);
    // set lowest point of MasterCoinped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (MasterCoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (MasterCoinpedData->at(1).y()-MasterCoinpedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (MasterCoinpedData->at(1).x()-MasterCoinpedData->at(0).x())/(MasterCoinpedData->at(1).y()-MasterCoinpedData->at(0).y());
    else
      slope = 0;
    (*MasterCoinpedData)[0].setX(MasterCoinpedData->at(0).x()+slope*(staticData->first().y()-MasterCoinpedData->at(0).y()));
    (*MasterCoinpedData)[0].setY(staticData->first().y());
    
    // MasterCoin upper bound:
    if (staticData->last().y() < MasterCoinpedData->last().y()) // other one must be MasterCoinped
      qSwap(staticData, MasterCoinpedData);
    int highBound = findIndexBelowY(MasterCoinpedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    MasterCoinpedData->remove(highBound+1, MasterCoinpedData->size()-(highBound+1));
    // set highest point of MasterCoinped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (MasterCoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = MasterCoinpedData->size()-1; // last index
    if (MasterCoinpedData->at(li).y()-MasterCoinpedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (MasterCoinpedData->at(li).x()-MasterCoinpedData->at(li-1).x())/(MasterCoinpedData->at(li).y()-MasterCoinpedData->at(li-1).y());
    else
      slope = 0;
    (*MasterCoinpedData)[li].setX(MasterCoinpedData->at(li-1).x()+slope*(staticData->last().y()-MasterCoinpedData->at(li-1).y()));
    (*MasterCoinpedData)[li].setY(staticData->last().y());
  }
