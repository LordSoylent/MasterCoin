...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMasterCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMasterCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMasterCoinAmount = strAnonymizeMasterCoinAmount.remove(strAnonymizeMasterCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMasterCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMasterCoinAmount*COIN) nMaxToAnonymize = nAnonymizeMasterCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMasterCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMasterCoinAmount));
        strAnonymizeMasterCoinAmount = strAnonymizeMasterCoinAmount.remove(strAnonymizeMasterCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMasterCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeMasterCoinAmount == 0){
