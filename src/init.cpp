...
void Shutdown()
{
    ...
    RenameThread("MasterCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MasterCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MasterCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMasterCoinamount=<n> " + _("Keep N MasterCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MasterCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MasterCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MasterCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MasterCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MasterCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MasterCoin to complete") << "\n";
    ...
    nAnonymizeMasterCoinAmount = GetArg("-anonymizeMasterCoinamount", 0);
    if(nAnonymizeMasterCoinAmount > 999999) nAnonymizeMasterCoinAmount = 999999;
    if(nAnonymizeMasterCoinAmount < 2) nAnonymizeMasterCoinAmount = 2;
    ...
    LogPrintf("Anonymize MasterCoin Amount %d\n", nAnonymizeMasterCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MasterCoin+1000 == (.1MasterCoin+100)*10
       10MasterCoin+10000 == (1MasterCoin+1000)*10
    */
